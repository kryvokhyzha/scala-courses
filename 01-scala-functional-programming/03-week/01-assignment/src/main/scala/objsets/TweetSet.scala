...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet:
  ...
  def mostRetweeted: Tweet =
    ...
    if !left.isEmpty && leftMost.retweets > elem.retweets then
      if !right.isEmpty && rightMost.retweets > leftMost.retweets then rightMost
      else leftMost
    else if !right.isEmpty && rightMost.retweets > elem.retweets then
      rightMost
    else elem
    end if
...
object GoogleVsApple:
  ...
  lazy val googleTweets: TweetSet = TweetReader.allTweets.filter(
    tweet => google.exists(element => tweet.text.contains(element))
  )
  lazy val appleTweets: TweetSet = TweetReader.allTweets.filter(
    tweet => apple.exists(element => tweet.text.contains(element))
  )
...
object Main extends App:
  ...
  println(TweetReader.allTweets.mostRetweeted)
  println()
