...
trait Huffman extends HuffmanInterface:
  ...
  def weight(tree: CodeTree): Int =
    tree match
      case Fork(_: CodeTree, _: CodeTree, _: List[Char], weight: Int) => weight
      case Leaf(_: Char, weight: Int) => weight

  def chars(tree: CodeTree): List[Char] =
    tree match
      case Fork(_: CodeTree, _: CodeTree, chars: List[Char], _: Int) => chars
  ...
  def times(chars: List[Char]): List[(Char, Int)] =
    chars match
      case Nil => Nil
      case _ => chars.groupBy(x => x).map(x => (x._1, x._2.size)).toList
  ...
  def makeOrderedLeafList(freqs: List[(Char, Int)]): List[Leaf] =
    freqs match
      case Nil => Nil
      case _ => freqs.sortBy(_._2).map((char, weight) => Leaf(char, weight))
  ...
  def encode(tree: CodeTree)(text: List[Char]): List[Bit] =
    def traverse(tree: CodeTree, char: Char): List[Bit] =
      tree match
        ...
        case Fork(_, right, _, _) => 1 :: traverse(right, char)

    text.flatMap(x => traverse(tree, x))
