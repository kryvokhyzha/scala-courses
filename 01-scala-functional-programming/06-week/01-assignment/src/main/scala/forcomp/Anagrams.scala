...
object Anagrams extends AnagramsInterface:
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] = occurrences match
    ...
    case (char, freq) :: xs =>
      for
        ...
        n <- 0 to freq
      yield if n == 0 then comb else (char, n) :: comb
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences =
    def subOccurrence(acc: Map[Char, Int], elem: (Char, Int)): Map[Char, Int] =
      val (charY, freqY) = elem
      val freqX = acc(charY)
      val freqDiff = freqX - freqY

      if freqDiff == 0 then acc - charY
      else acc + (charY -> freqDiff)
    y.foldLeft(x.toMap)(subOccurrence).toList.sorted
  ...
  def sentenceAnagrams(sentence: Sentence): List[Sentence] =
    def subSentence(occ: Occurrences): List[Sentence] = {
      if (occ.isEmpty) List(List())
        ...
        for
          x <- combs if dictionaryByOccurrences.keySet(x)
